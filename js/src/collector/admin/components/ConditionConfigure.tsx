...
export default class ConditionConfigure extends Component<{ conditions: Stream<ConditionData[]> }> {
    ...
    view(vnode: any) {
        return <div className="condition-table-wrapper">
                    ...
                    {this.conditions.map((item, index) => {
                        return (
                            <tr key={`${item.name}-${index}`}>
                                ...
                                <td>
                                    <input className="FormControl" type="number" value={item.value} onchange={((e: InputEvent) => {
                                ...
                                <td>
                                    <input className="FormControl" type="number" min="1" value={item.span || ""} onchange={((e: InputEvent) => {
                                        const value = parseInt((e.target as HTMLInputElement).value, 10);
                                ...
                                <td>
                                    <div className="condition-table-actions">
                                        {showIf(item.name != '*',
                                            <Button className="Button Button--danger Button--icon" aria-label={app.translator.trans('xypp-collector.admin.list.condition-delete')} onclick={((e: any) => {
                                        ...
                                        {showIf((this.conditions[index - 1] && item.name != '*'),
                                            <Button className="Button Button--secondary Button--icon" aria-label={app.translator.trans('xypp-collector.admin.list.condition-move-up')} onclick={this.swap(index, -1)}>
                                                <i class="fas fa-sort-up"></i>
                                            </Button>
                                        )}
                                        {showIf((this.conditions[index + 2] && item.name != '*'),
                                            <Button className="Button Button--secondary Button--icon" aria-label={app.translator.trans('xypp-collector.admin.list.condition-move-down')} onclick={this.swap(index, 1)}>
